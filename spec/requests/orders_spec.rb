...
RSpec.describe "Orders", type: :request do
  ...
  #     order = FactoryBot.create(:order)
  #     put order_path(order.id), params: { order: { product_count: 2000 } }
  #     order.reload
  #     expect(order.product_count).to eq(2000)
  ...
  describe "put order_path with invalid data" do
    it "does not update the order record or redirect" do
      order = FactoryBot.create(:order)
      put order_path(order.id), params: { order: { product_count: "aaa" } }
      order.reload
      expect(order.product_count).not_to eq("aaa")
      expect(response).to render_template(:edit)
    end
  end
